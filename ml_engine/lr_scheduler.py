...
def build_scheduler(conf, epochs, optimizer, n_iter_per_epoch):
    ...
    if conf.name == 'cosine':
        lr_scheduler = CosineLRScheduler(
            optimizer,
            t_initial=(num_steps - warmup_steps) if conf.warmup_prefix else num_steps,
            lr_min=conf.min_lr,
            warmup_lr_init=conf.warmup_lr,
            warmup_t=warmup_steps,
            cycle_limit=1,
            t_in_epochs=False,
            warmup_prefix=conf.warmup_prefix,
        )
    elif conf.NAME == 'linear':
